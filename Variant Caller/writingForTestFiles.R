writeArffForTestFiles = function(folder){
  ...
  mutect2 <- Sys.glob(file.path(getwd(), "*mutect*.vcf.gz"))
  ...
  mutect2.tbi <- Sys.glob(file.path(getwd(), "*mutect*gz.tbi"))
  ...
  if (folder == "real2_part1"){
    ...
    parse_snv = merge(x=parse_snv,y=bed,all.x=TRUE)
    # #non bed data replaced with FALSE
    parse_snv$TRUTH[is.na(parse_snv$TRUTH)]= FALSE
  } else{
    parse_snv['TRUTH'] = NA
  }
  ...
  write.arff(parse_snv, file = paste0(getwd(), "/", folder, "_test.arff"), eol = "\n")
  return(parse_snv)
}


setwd("C:/Users/mrlim/OneDrive/Desktop/NUS/Y2S2/CS4220/Data2")
file = writeArffForTestFiles("real2_part1")
